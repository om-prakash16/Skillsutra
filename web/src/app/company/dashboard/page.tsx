    ...
    if (isLoading) {
        return (
            <div className="flex h-[60vh] w-full flex-col items-center justify-center gap-4">
                ...
                <p className="text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/50">Initializing Neural Pipeline...</p>
            </div>
        )
    }
        ...
        <div className="space-y-10 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((stat, i) => (
                    <Card key={i} className="bg-muted/50 border-border hover:border-primary/30 transition-all group relative overflow-hidden backdrop-blur-xl">
                        ...
                        <CardContent className="pt-2">
                            ...
                            <p className="text-[9px] text-muted-foreground/50 mt-1 font-medium">{stat.sub}</p>
                        </CardContent>
                    </Card>
                ))}
            </div>
            ...
            <div className="grid gap-8 md:grid-cols-3">
                {/* Recent Activity */}
                <Card className="col-span-2 bg-muted/50 border-border backdrop-blur-xl relative overflow-hidden">
                    ...
                    <CardContent>
                        <div className="space-y-1">
                            {activity.length === 0 ? (
                                <div className="text-center py-10 text-muted-foreground/50 text-xs italic">No active signals detected.</div>
                            ) : activity.map((item, idx) => (
                                <div key={idx} className="flex items-center p-3 rounded-xl hover:bg-muted/50 transition-all group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold text-foreground/90 truncate">{item.description}</p>
                                    ...
                                    <ArrowRight className="w-3 h-3 text-muted-foreground/30 group-hover:text-primary transition-colors opacity-0 group-hover:opacity-100" />
                                </div>
                            ))}
                        </div>
                    </CardContent>
                ...
                <div className="space-y-6">
                    <h3 className="text-sm font-black uppercase tracking-[0.25em] px-2 text-muted-foreground italic">Quick Commands</h3>
                    <div className="grid gap-3">
                        <Link href="/company/jobs" className="block">
                            <Button variant="outline" className="w-full justify-between h-14 px-4 bg-muted/50 border-border hover:bg-white/[0.08] hover:border-primary/30 group px-5 rounded-2xl">
                        ...
                        <Card className="p-6 bg-emerald-500/5 border-emerald-500/20 rounded-3xl relative overflow-hidden group hover:bg-emerald-500/10 transition-all cursor-pointer">
                            ...
                            <div className="flex items-baseline gap-2 mb-2">
                                ...
                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Unseen Stars Found</span>
                            </div>
                            <p className="text-[10px] text-muted-foreground leading-relaxed italic mb-4">AI has identified high-resonance candidates for your active missions who haven't applied yet.</p>
                        ...
                        <Card className="p-6 bg-primary/5 border-primary/20 rounded-3xl mt-4">
                            ...
                            <p className="text-xs text-foreground/80 leading-relaxed italic">"Matching precision improved by 14% this week. Your 'Senior Rust' listing is gaining traction in the European cluster."</p>
                        </Card>
                    </div>
                </div>
            </div>
