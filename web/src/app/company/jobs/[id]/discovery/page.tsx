        ...
        return (
            <div className="flex h-[80vh] w-full flex-col items-center justify-center gap-6">
                ...
                <div className="text-center space-y-2">
                    <p className="text-[10px] uppercase tracking-[0.4em] font-black text-muted-foreground">Analyzing Talent Clusters</p>
        ...
        <div className="max-w-7xl mx-auto space-y-10 pb-20 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-border/50">
                ...
                <div className="flex gap-4">
                    <Card className="bg-muted/50 border-border px-6 py-4 rounded-2xl flex items-center gap-4">
                        <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center">
                            <Users className="w-5 h-5 text-primary" />
                        </div>
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Matches Found</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {candidates.length === 0 ? (
                    <div className="col-span-full border border-dashed border-border rounded-3xl p-20 text-center bg-white/[0.01]">
                    ...
                    <Card key={idx} className="bg-muted/50 border-border hover:border-primary/40 transition-all rounded-3xl overflow-hidden group">
                        <CardHeader className="pb-4 relative">
                            <div className="flex justify-between items-start gap-4">
                                <div className="flex gap-5">
                                    <div className="w-16 h-16 rounded-2xl bg-muted/50 border border-border flex items-center justify-center overflow-hidden relative group-hover:scale-105 transition-transform">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-foreground/80">Reputation: {candidate.reputation}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    ...
                                    <p className="text-[9px] uppercase font-black tracking-widest text-muted-foreground/50">Proof Score Resonance</p>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            ...
                            <div className="bg-primary/5 rounded-2xl p-4 border border-primary/10 relative overflow-hidden">
                                ...
                                <p className="text-sm italic text-foreground/90 leading-relaxed font-medium">"{candidate.match_reason}"</p>
                            </div>

                            {/* Skills Tag Stream */}
                            <div className="space-y-3">
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Top Skill Clusters</h4>
                                <div className="flex flex-wrap gap-2">
                                    {candidate.skills?.slice(0, 6).map((skill: string, i: number) => (
                                        <Badge key={i} variant="secondary" className="bg-muted/50 border-border/50 hover:bg-muted/50 text-[10px] font-bold uppercase tracking-wide px-3 py-1">
                                            {skill}
                                        </Badge>
                                    ))}
                                    {candidate.skills?.length > 6 && (
                                        <Badge variant="ghost" className="text-[10px] font-black text-muted-foreground/50">+{candidate.skills.length - 6} more</Badge>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3 pt-4">
                                ...
                                <Button variant="outline" size="icon" className="h-12 w-12 border-border hover:bg-muted/50">
