    ...
    if (companyLoading || jobsLoading) {
        return (
            <div className="flex h-[60vh] w-full flex-col items-center justify-center gap-4">
                ...
                <p className="text-[10px] uppercase tracking-[0.4em] font-black text-muted-foreground/50">Syncing Command Center Stream...</p>
            </div>
        )
    }
        ...
        <div className="max-w-6xl mx-auto space-y-12 pb-24 animate-in fade-in duration-1000">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border/50 pb-10">
                <div className="space-y-4">
                    ...
                    <h1 className="text-5xl md:text-6xl font-black font-heading tracking-tighter uppercase italic text-foreground">
                ...
                <Link href="/company/post-job">
                    <Button size="lg" className="h-16 px-8 bg-white text-primary-foreground hover:bg-neutral-200 font-black tracking-tighter uppercase gap-3 shadow-xl shadow-white/5">
            ...
            {/* Missions List */}
            <Card className="bg-muted/50 border-border backdrop-blur-3xl overflow-hidden shadow-2xl relative">
                ...
                <CardHeader className="p-8 border-b border-border/50">
                    <div className="flex items-center justify-between">
                        <div className="space-y-1">
                            ...
                            <CardDescription className="text-xs uppercase font-black tracking-widest text-muted-foreground">Personnel synchronization across the network</CardDescription>
                        </div>
                        <div className="flex gap-4">
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-muted/50 border border-border">
                                <Wifi className="w-3.5 h-3.5 text-emerald-400" />
                                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Signal Strength: 98%</span>
                            </div>
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="p-0">
                    {!jobs || jobs.length === 0 ? (
                        <div className="text-center py-20">
                            ...
                            <p className="text-muted-foreground/50 font-black italic text-lg tracking-tight uppercase">No active missions detected. Initiate first launch to begin.</p>
                        </div>
                    ) : (
                        <Table>
                            <TableHeader className="bg-muted/30">
                                <TableRow className="border-border/50 hover:bg-transparent px-8">
                                    <TableHead className="font-black uppercase tracking-widest text-[9px] text-muted-foreground h-14 pl-8">Mission Designation</TableHead>
                                    <TableHead className="font-black uppercase tracking-widest text-[9px] text-muted-foreground h-14">Current Pulse</TableHead>
                                    <TableHead className="font-black uppercase tracking-widest text-[9px] text-muted-foreground h-14 text-center">Applicants</TableHead>
                                    <TableHead className="font-black uppercase tracking-widest text-[9px] text-muted-foreground h-14 text-center">AI Discoveries</TableHead>
                                    <TableHead className="font-black uppercase tracking-widest text-[9px] text-muted-foreground h-14 text-right pr-8">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {jobs.map((job: any) => (
                                    <TableRow key={job.id} className="border-border/50 hover:bg-muted/30 group transition-colors px-8 h-20">
                                        <TableCell className="pl-8 font-black text-lg text-foreground group-hover:text-primary transition-colors">
                                            {job.title}
                                            <div className="flex gap-2 mt-1.5">
                                                <Badge variant="outline" className="text-[8px] font-black uppercase text-muted-foreground border-border bg-muted/50">{job.job_type || 'Full-time'}</Badge>
                                                {job.has_assessment && (
                                        ...
                                        <TableCell className="text-center">
                                            <Link href={`/company/jobs/${job.id}/applicants`}>
                                                <div className="inline-flex flex-col items-center gap-1 group/apps cursor-pointer">
                                                    <span className="text-2xl font-black text-foreground group-hover/apps:text-primary transition-colors">{job.applicant_count || 0}</span>
                                                    <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/50 group-hover/apps:text-primary/40 transition-colors">Total Personnel</span>
                                                </div>
                                            </Link>
                                        </TableCell>
                                        <TableCell className="text-center">
                                        ...
                                        <TableCell className="text-right pr-8">
                                            <div className="flex justify-end gap-3 opacity-30 group-hover:opacity-100 transition-opacity">
                                                <Button variant="ghost" size="icon" asChild title="Global Preview" className="h-10 w-10 bg-muted/50 hover:bg-muted/50 border border-border">
