...
export default function ModeratorsPage() {
    ...
    const fetchModerators = async () => {
        setLoading(true)
        try {
            const data = await api.get(`/admin/users?role=moderator&limit=100`)
            setModerators(data?.data?.users || [])
        } catch (err) {
        ...
        <div className="space-y-10 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-border/50 pb-10">
                <div className="space-y-3">
                    <div className="flex items-center gap-3">
                        ...
                        <Badge variant="outline" className="glass border-orange-500/30 text-orange-400 px-4 font-black tracking-widest text-[9px] uppercase rounded-full">
                            Trust & Safety
                        </Badge>
                    </div>
                    <h1 className="text-4xl md:text-5xl font-black font-heading tracking-tighter text-foreground">
                        Platform <span className="text-orange-500">Moderators</span>
                    </h1>
                    <p className="text-muted-foreground text-lg max-w-xl">
                        Manage the trust and safety team members who monitor and enforce community guidelines.
                    </p>
                </div>
                ...
                <div className="flex gap-3">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-10 pr-4 py-2 w-full md:w-64 h-11 rounded-xl bg-background/50 border border-border text-sm text-foreground focus:outline-none focus:border-orange-500/50 transition-all"
                        />
                    </div>
                    <ProvisionUserModal
                        role="moderator"
                        ...
                        trigger={
                            <Button className="rounded-xl font-bold bg-orange-500 hover:bg-orange-600 text-white gap-2 px-6 h-11">
            ...
            {loading ? (
                <div className="flex items-center justify-center py-24">
                    <Loader2 className="w-10 h-10 animate-spin text-orange-500/40" />
                </div>
            ) : filtered.length === 0 ? (
                <Card className="glass border-border/50 rounded-[2rem]">
                    <CardContent className="flex flex-col items-center justify-center py-20 gap-4">
                        <ShieldAlert className="w-16 h-16 text-orange-500/30" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filtered.map((mod, idx) => (
                        ...
                        >
                            <Card className="glass border-border/50 rounded-[1.5rem] overflow-hidden hover:border-orange-500/20 transition-all duration-300">
                                <CardContent className="p-6">
                                    <div className="flex items-start justify-between gap-4">
                                        ...
                                        <Badge variant="outline" className={mod.status === 'active' ? "text-emerald-400 border-emerald-400/30 bg-emerald-500/5" : "text-zinc-400 border-zinc-400/30 bg-zinc-500/5"}>
                                            {mod.status}
                                        </Badge>
                                    </div>
