            ...
            <div className="grid grid-cols-1 gap-6">
                {mockTickets.map((ticket, i) => (
                    <motion.div
                        ...
                        className="group relative bg-muted/50 border border-border/50 hover:border-border rounded-[2.5rem] p-8 backdrop-blur-xl transition-all cursor-pointer overflow-hidden"
                    >
                        {/* Status Accent */}
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                            <div className="flex flex-col gap-4 flex-1">
                                <div className="flex items-center gap-3">
                                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground">{ticket.id}</span>
                                    ...
                                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">{ticket.time}</span>
                                </div>
                                <h3 className="text-2xl font-black font-heading tracking-tight text-foreground/90 group-hover:text-primary transition-colors">
                                    {ticket.subject}
                                </h3>
                                <div className="flex items-center gap-4">
                                    <div className="flex items-center gap-2 px-3 py-1 bg-muted/50 rounded-full border border-border/50">
                                        <User className="w-3.5 h-3.5 text-muted-foreground" />
                                        <span className="text-xs font-bold text-foreground/80 tracking-tight">{ticket.user}</span>
                                    </div>
                                    <div className="flex items-center gap-2 px-3 py-1 bg-muted/50 rounded-full border border-border/50">
                                        ...
                                        <span className="text-xs font-bold text-foreground/80 tracking-tight uppercase">{ticket.status}</span>
                                    </div>
                                </div>
                            </div>

                            <button className="flex items-center justify-center gap-4 px-10 py-5 bg-muted/50 border border-border/50 group-hover:bg-primary group-hover:border-primary group-hover:text-primary-foreground rounded-3xl transition-all duration-500 min-w-[200px]">
