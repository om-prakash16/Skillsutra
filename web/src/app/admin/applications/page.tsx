    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="relative group">
          ...
          <Input 
            ...
            className="h-14 bg-muted/50 border-border pl-14 text-lg focus-visible:ring-primary/30 rounded-2xl"
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
      </div>

      <Card className="bg-muted/50 border-border backdrop-blur-xl shadow-2xl relative overflow-hidden group border-t-primary/30 border-t-2">
        ...
        <CardHeader className="relative z-10 border-b border-border flex flex-row items-center justify-between pb-6">
            ...
            <Badge variant="outline" className="bg-muted/50 border-border font-mono tracking-wider text-primary uppercase">
          ...
          <Table>
            <TableHeader className="bg-black/20">
              <TableRow className="border-border hover:bg-transparent">
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-muted-foreground h-14 px-6">Candidate Vector</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-muted-foreground h-14">Target Listing</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-muted-foreground h-14">Match Resonance</TableHead>
                ...
                >
                  <TableCell className="px-6 py-5">
                      <div className="flex flex-col">
                          <span className="font-bold text-foreground tracking-tight font-mono text-xs italic">{app.users?.wallet_address.substring(0,8)}...{app.users?.wallet_address.substring(app.users?.wallet_address.length-4)}</span>
                          <span className="text-[10px] text-muted-foreground uppercase mt-1">Applied {new Date(app.created_at).toLocaleDateString()}</span>
                      </div>
                  </TableCell>
                  <TableCell>
                      <span className="text-xs font-black uppercase text-foreground/90 tracking-widest">{app.jobs?.title || 'GENERIC_JOB'}</span>
                  </TableCell>
                  <TableCell>
                      <div className="flex items-center gap-3">
                          <div className="h-1.5 flex-1 bg-muted/50 rounded-full overflow-hidden max-w-[100px]">
                              <div 
                          ...
                          <span className="text-xs font-black italic text-foreground/80">{app.ai_match_score || 0}%</span>
                      </div>
                  </TableCell>
                  <TableCell>
