...
export function TalentCard({ talent }: TalentCardProps) {
  ...
  return (
    <Card className="group relative overflow-hidden glass border-border/50 hover:border-primary/40 transition-all duration-500 rounded-[2.5rem] shadow-2xl hover:shadow-primary/5">
      <div className="p-8 space-y-8 relative">
            ...
            <div className="relative shrink-0">
                <div className="w-16 h-16 rounded-[1.25rem] bg-gradient-to-tr from-primary/20 to-indigo-500/20 p-0.5 group-hover:scale-105 transition-transform duration-500 shadow-lg">
                    <Avatar className="w-full h-full rounded-[1.2rem] border-none">
                        ...
                        <AvatarFallback className="bg-muted text-primary font-black italic">
                            {getInitials(talent.name)}
                        </AvatarFallback>
                    </Avatar>
                </div>
                {talent.verified && (
                    <div className="absolute -bottom-1 -right-1 bg-background rounded-full p-1 border border-border shadow-lg">
        ...
        <div className="space-y-3">
            <div className="flex justify-between text-[9px] uppercase font-black tracking-[0.3em] text-muted-foreground/50">
                <span>Node Completion</span>
                <span className="text-muted-foreground">{talent.completion}%</span>
            </div>
        ...
        <div className="flex items-center gap-6">
            <div className="flex items-center text-[9px] uppercase font-black tracking-widest text-muted-foreground">
        ...
        <div className="pt-2 flex flex-wrap gap-2">
            {talent.skills.slice(0, 3).map(skill => (
                <Badge key={skill} variant="outline" className="glass border-border text-[9px] font-black uppercase tracking-widest text-muted-foreground px-3 py-1 rounded-lg group-hover:border-primary/20 group-hover:text-primary/60 transition-colors">
                    {skill}
                </Badge>
            ))}
            {talent.skills.length > 3 && (
                <Badge variant="outline" className="glass border-border text-[9px] font-black uppercase tracking-widest text-muted-foreground/50 px-3 py-1 rounded-lg">
                    +{talent.skills.length - 3}
                </Badge>
            )}
        </div>
