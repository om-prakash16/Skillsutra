...
export function FeaturedArticleCard({ article }: { article: any }) {
    return (
        <div className="p-8 border border-primary/20 rounded-3xl bg-primary/5">
            ...
            <h3 className="text-2xl font-black tracking-tighter italic mb-4">{article?.title || "Mastering the infrastructure Ecosystem"}</h3>
            <p className="text-xs text-foreground/80 mb-6 italic">Strategy and telemetry for elite NextGen engineering roles.</p>
        </div>
    )
}
