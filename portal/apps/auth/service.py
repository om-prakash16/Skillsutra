...
from portal.integrations.blockchain.service import verify_blockchain_signature
...
class AuthService:
    ...
    async def authenticate_wallet(self, wallet_address: str, message: str, signature: str):
        is_valid = await verify_blockchain_signature(wallet_address, message, signature)
        if not is_valid:
