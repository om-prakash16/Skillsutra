...
logger = ProtocolLogger.get_logger("modules.jobs.repos")
...
class JobRepository:
    ...
    async def list_jobs(self, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise ExternalServiceError(message="Failed to retrieve jobs")

    async def get_by_id(self, job_id: str) -> Dict[str, Any]:
        """Fetch a single active job by ID."""
        try:
            res = await self._base_query().select("*, companies(*)").eq("id", job_id).single().execute()
            if not res.data:
    ...
    async def create(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            res = await self.db.table(self.table).insert(data).execute()
            if not res.data:
    ...
    async def list_by_company(self, company_id: str) -> List[Dict[str, Any]]:
        ...
        res = await self._base_query().eq("company_id", company_id).execute()
        return res.data or []
