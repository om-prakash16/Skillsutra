...
from modules.users.core.identity_service import IdentityService
from modules.users.core.identity_proof_service import IdentityProofService
...
async def submit_id(
    ...
    """
    try:
        return await proof_service.submit_identity(
            current_user["sub"], id_type, document_url
        )
...
async def update_privacy(
    visibility: str = Body(..., embed=True), current_user=Depends(get_current_user)
):
    """
    ...
    try:
        ...
        return await identity_service.update_privacy_settings(
            current_user["sub"], visibility
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def request_connection(
    target_user_id: str = Body(..., embed=True), current_user=Depends(get_current_user)
):
    """
    Send a connection request to another professional.
    """
    try:
        return await identity_service.request_connection(
            current_user["sub"], target_user_id
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
