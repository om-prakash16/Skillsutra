...
async def mint_profile(data: MintProfileRequest):
    ...
    """
    metadata = await nft_service.generate_profile_metadata(
        data.user_id, data.attributes
    )
...
async def mint_skill(data: MintSkillRequest):
    ...
    """
    verification = await nft_service.check_skill_verification(
        data.user_id, data.skill_name
    )
    if not verification:
        raise HTTPException(
            status_code=400, detail="Skill verification failed. Take the AI quiz first."
        )

    metadata = await nft_service.generate_skill_metadata(
        data.user_id, data.skill_name, verification["score"], verification["level"]
    )
    cid = await nft_service.upload_to_ipfs(metadata)
