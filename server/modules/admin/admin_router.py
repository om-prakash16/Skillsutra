...
async def update_user(
    ...
    response = (
        db.table("users").update(update_data).eq("wallet_address", wallet).execute()
    )
...
async def create_skill(
    skill: SkillCategoryCreate, user=Depends(require_permission("manage_schema"))
):
    db = get_supabase()
...
async def update_subscription(
    ...
    user=Depends(require_permission("manage_flags")),
):
    db = get_supabase()
    update_data = {k: v for k, v in payload.items() if v is not None}
    response = (
        db.table("platform_subscriptions")
        .update(update_data)
        .eq("id", sub_id)
        .execute()
    )
