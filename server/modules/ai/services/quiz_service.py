...
class QuizService:
    ...
    async def generate_quiz(
        self, skill: str, difficulty: str = "intermediate", count: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not self.llm:
            ...
            return [
                {
                    "id": "q1",
                    "text": f"What is a key concept in {skill}?",
                    "options": ["A) X", "B) Y", "C) Z", "D) W"],
                    "correct": "C",
                },
                {
                    "id": "q2",
                    "text": f"How do you handle error in {skill}?",
...
[{{"id": "q1", "text": "...", "options": ["A) ...", "B) ...", "C) ...", "D) ..."], "correct": "C"}}]""",
        ...
        try:
            result = self.llm.invoke(
                prompt.format(count=count, skill=skill, difficulty=difficulty)
            )
    ...
    async def evaluate(
        self, quiz_id: str, user_answers: Dict[str, str]
    ) -> Dict[str, Any]:
        """
        ...
        response = (
            self.db.table("ai_quizzes").select("*").eq("id", quiz_id).single().execute()
        )
        ...
        if passed:
            ...
            self.db.table("ai_quizzes").update(
                {"passed": True, "score": score, "mint_token": mint_token}
