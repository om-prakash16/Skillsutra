...
class GitHubEvaluator:
    ...
    async def get_user_score(self, username: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            try:
                # 1. Fetch repos
                repos_resp = await client.get(f"{self.base_url}/users/{username}/repos?per_page=100&sort=updated")
                if repos_resp.status_code != 200:
                    return {"error": "User not found", "score": 0}
                
                repos = repos_resp.json()
                if not repos:
                    return {"username": username, "score": 0, "message": "No repositories found"}
                
                # 2. Basic Metrics
                total_stars = sum(r.get("stargazers_count", 0) for r in repos)
                total_forks = sum(r.get("forks_count", 0) for r in repos)
                unique_langs = list(set(r.get("language") for r in repos if r.get("language")))
                ...
                comp_score = ai_insights.get("complexity_score", min(len(repos) * 10, 80)) if ai_insights else min(len(repos) * 10, 80)
                var_score = ai_insights.get("variety_score", min(len(unique_langs) * 20, 80)) if ai_insights else min(len(unique_langs) * 20, 80)
                pop_score = min(total_stars * 10, 100)

                final_score = int((comp_score * 0.4) + (pop_score * 0.3) + (var_score * 0.3))
                ...
                return {
                    ...
                    "ai_evaluation": ai_insights.get("summary") if ai_insights else "Standard statistics-based profile.",
                    "primary_stack": ai_insights.get("primary_tech_stack") if ai_insights else unique_langs[:3]
                }
            except Exception as e:
                return {"error": str(e), "score": 0}

    def _get_level(self, score: int) -> str:
        if score >= 85: return "Architect"
